...
export const strictIssueIdRegEx = /(?<=^|[a-z]-|[\s&P[\]^cnptu{}\-])([A-Za-z]\w*[ \-]\d+)(?![^\W_])/g;
export const issueIdRegEx = /([A-Z]\w+[ _-]\d+)/g;
/**
 * @param value The string to transform to title case.
 */
export function TitleCasePipe(value: string | null | undefined): string | null {
...
export function upperCaseFirst(line: string): string {
  return replace(line, /\w\S*/g, (txt) => toUpper(txt.charAt(0)) + txt.slice(1));
}
